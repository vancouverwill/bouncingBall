...
gulp.task('html', () => {
  ...
  .on('end', function(){ gutil.log('Done! Save ' + config.paths.html + ' in ' + config.paths.baseDir); });
});
...
gulp.task('css', () => {
  ...
  .pipe(gulp.dest(config.paths.baseDir))
  .pipe(cond(!PROD, livereload()))
  .on('end', function(){ gutil.log('Done! Save ' + config.paths.css + ' in ' + config.paths.baseDir); });
});
